...
public class LeftAlienScript : MonoBehaviour
{
    ...
    void Start()
    {
        LeftTarget = WayPointScript.LeftPoints[0];
    ...
    void Update()
    {
        Vector3 dir = LeftTarget.position - transform.position;
        transform.Translate(dir.normalized * monSpeed * Time.deltaTime, Space.World);

        if (Vector3.Distance(transform.position, LeftTarget.position) <= 0.2f)
        {
            getNextWayPoint();
        }
    ...
    {
        if (pointCount < WayPointScript.LeftPoints.Length)
        {
            transform.LookAt(WayPointScript.LeftPoints[pointCount]);
            LeftTarget = WayPointScript.LeftPoints[pointCount];
            pointCount++;
        }
    }
